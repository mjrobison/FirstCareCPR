...
function em_content($page_content) {
	...
	if( !post_password_required() && in_array($post->ID, array($events_page_id, $locations_page_id, $categories_page_id, $edit_bookings_page_id, $edit_events_page_id, $edit_locations_page_id, $my_bookings_page_id, $tags_page_id)) ){
		...
		return apply_filters('em_content', '<div class="container"><p>The events found here are public events, to schedule a private class please contact us at firstcarecpr@gmail.com, or through the <a href="/wordpress/contact/">contact form.</a></p>'.$content.'</div>');
	}
	return $page_content;
}
